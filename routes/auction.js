...
router.post(
  ...
  async (req, res) => {
    try {
      const { name, description, entry_bid_points, minimum_users, category } =
        req.body;
...
router.get("/", authenticateToken, async (req, res) => {
  try {
    ...
    const adjustedAuctions = auctions.map((auction) => {
      if (auction.image) {
        auction.image = `${req.protocol}://${req.get("host")}${auction.image}`;
      }
      return auction;
    });

    res.status(200).json(adjustedAuctions);
  } catch (error) {
